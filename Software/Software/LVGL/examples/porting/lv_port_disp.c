 ...
 *  STATIC VARIABLES
 **********************/
static volatile uint32_t flush_count = 0U;
static volatile uint32_t flush_enter_count = 0U;
...
uint32_t lv_port_disp_get_flush_enter_count(void)
{
    return flush_enter_count;
}
...
static void disp_flush(lv_disp_drv_t * disp_drv, const lv_area_t * area, lv_color_t * color_p)
{
    ...
    if(disp_flush_enabled) {
        ...
        ILI9341_DrawRGB565((uint16_t)area->x1,
                           (uint16_t)area->y1,
                           width,
                           height,
